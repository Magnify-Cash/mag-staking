...
contract MAGTieredStaking is ReentrancyGuard, Ownable, Pausable {
    ...
    function getAPYForLockPeriod(uint256 lockPeriod) public view returns (uint256) {
    ...
    function calculateRewards(address user) public view returns (uint256) {
        ...
        return (stakeInfo.amount * stakeInfo.apy * timeElapsed) / (365 days * PERCENTAGE_DENOMINATOR);
    }
    
    // Core functions
    function stake(uint256 amount, uint256 lockPeriodInDays) external nonReentrant whenNotPaused {
        require(amount > 0, "Cannot stake 0 tokens");
        require(!stakes[msg.sender].initialized || stakes[msg.sender].amount == 0, "Existing stake found");
        ...
        require(magToken.transferFrom(msg.sender, address(this), amount), "Transfer failed");
    ...
    function unstake() external nonReentrant {
        ...
        require(stakeInfo.initialized && stakeInfo.amount > 0, "No stake found");
        ...
        if (rewards > 0) {
            require(magToken.transfer(msg.sender, rewards), "Rewards transfer failed");
        }
    ...
    // Admin functions
    function addStakingTier(uint256 lockPeriod, uint256 apy) external onlyOwner {
        require(lockPeriod > 0, "Invalid lock period");
        require(apy > 0 && apy <= 3000, "Invalid APY"); // Max 30% APY
        
        for (uint256 i = 0; i < stakingTiers.length; i++) {
            require(stakingTiers[i].lockPeriod != lockPeriod, "Lock period already exists");
        }
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        require(tokenAddress != address(magToken), "Cannot recover staking token");
